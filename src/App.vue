...
<template>
  ...
  <div class="flex justify-center -ml-12">
    <div
      ...
      :class="{
        ...
        'animate-pulse': isConnection(['ONLINE', 'CONNECTING']) && isQueue(['WAITING_FOR_OPPONENT', 'REQUESTING_GAME']),
      }"
    ></div>
    <GameBoard
  ...
  <div class="flex flex-col justify-start space-y-1 h-28">
    ...
    <div class="flex justify-center" v-if="isConnection('ONLINE') && isQueue('REQUESTING_GAME')">
      Requesting game...
    </div>
...
export default {
  ...
  methods: {
    ...
    isConnection(state: string | string[]): boolean {
      ...
      console.log(state.includes(this.connectionState));
    ...
    isQueue(state: string | string[]): boolean {
      ...
      console.log(state.includes(this.queueState));
